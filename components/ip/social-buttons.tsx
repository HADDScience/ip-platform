 ...
 *  - 카카오: #FEE500 고정, 검정 말풍선 심볼과 검정 텍스트(불투명도 85%)
 *
...
const BASE =
  "relative flex h-11 w-full items-center justify-center gap-2 rounded-md px-4 text-[13px] font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-60 focus-visible:outline-2 focus-visible:outline-offset-2"

export function GoogleSignInButton({ onClick, disabled, pending }: Props) {
  return (
    <button
      type="button"
      onClick={onClick}
...
export function KakaoSignInButton({ onClick, disabled, pending }: Props) {
  return (
    <button
      type="button"
      onClick={onClick}
      ...
      className={cn(
        ...
        "bg-[#FEE500] text-black/85 hover:bg-[#f2da00] focus-visible:outline-[#FEE500]"
      )}
    >
      <span className="absolute left-4 flex items-center">
        <KakaoMark />
      </span>
      {pending ? "이동 중…" : "카카오 로그인"}
    </button>
  )
}
