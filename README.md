...
- **Google 또는 카카오** 소셜 로그인만 지원합니다.
- 허브(`/hub/`)와 같은 오리진이라 **한 번 로그인하면 세션이 공유**됩니다.
...
| 화면 | 내용 |
| --- | --- |
| **대시보드** | 상표·특허 상태별 집계, 미결 액션/커뮤니케이션 수, 90일 이상 정체 경고, 오래 정체된 건 TOP 8, 정합성 경고 요약 |
| **상표** | 상태·류·출원인 필터, 검색, 정렬. 행 클릭 → 추가/수정/삭제 |
| **특허** | 동일. 사건번호·출원번호 관리 |
| **커뮤니케이션 로그** | 시간순 타임라인, 미결 배지, 첨부 파일명, **Gmail 원문 딥링크**, 상표/특허 연결 |
| **미결 액션** | 우선순위 보드/목록, 경과일, **완료 처리** |
| **정합성 경고** | 「※」 확인 필요 항목의 **해결 처리**, 출원번호 중복 자동 감지 |

날짜는 KST 기준이고, 경과일은 브라우저에서 실제 오늘 기준으로 계산합니다.

## 입력을 빠르게 하는 장치

- **메일 붙여넣기 자동 채우기** — 커뮤니케이션 기록 추가 시 메일 본문을 그대로 붙여넣으면
  일자·발신·수신·제목·본문·첨부를 파싱해 폼을 채웁니다. **Gmail 과 네이버웍스 모두** 동작합니다
  (OAuth 를 쓰지 않기 때문입니다).
- **Gmail 원문 열기** — `gmail_thread_id` 가 있으면 타임라인에서 원문으로 바로 이동합니다.
- **⌘/Ctrl + Enter** 로 저장, ID 자동 채번(TM-22, PT-11, A-9 …).
- 비고에 `※` 로 시작하는 문장을 쓰면 **정합성 경고**에 자동으로 모입니다.
